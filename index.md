...
By default the app writes to `app/output/`. Point it somewhere else
with:
...
`app/R/cohort_kinds.R`). An older file’s declared columns are dropped on
...
pipelines*.
[`omopgenerics::bind()`](https://darwin-eu.github.io/omopgenerics/reference/bind.html)
joins their tables, renumbering the cohort ids in argument order, so one
...
Each type’s inputs live in their own file,
`app/R/analysis_type_<name>.R`, which calls
`register_analysis_template()` with four mirrored pieces: `ui` builds
the inputs, `collect` reads them back into JSON, `pickers` names the ids
that pick a cohort or a CDM source, and `flatten` undoes whatever
nesting `collect` did so a saved file can repopulate the form. Copy an
existing one and edit it — that is the whole wiring, no other file needs
touching.
...
are filled in. `app/R/analysis_registry.R` holds the registry, the type
...
the template files have to sit flat in `app/R/`.

## Layout

The repo holds two things: the **package** at the root — the versioned
SAP schema, its constructors and its validators — and the **Shiny app**
in `app/`, which is a Shiny app *directory*, not a package. The app is
frozen pending a rewrite against the schema; it does not depend on the
package, and the package does not depend on it.
...
Launch the app with `shiny::runApp("app")` from the repo root.
`runApp()` sets the working directory to `app/` for the duration, which
is what makes the app’s relative paths resolve.
