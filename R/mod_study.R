...
amendment_item_ui <- function(id, prefill = NULL) {
  ...
  item_card(
    ...
    bslib::layout_columns(
      col_widths = c(3, 3, 6),
      shiny::textInput(ns("version"), "Version", pf("version"), width = "100%"),
      date_input(ns("date"), "Date", pf("date")),
      shiny::div()
    ),
    shiny::textAreaInput(ns("description"), "Description of change", pf("description"),
                  rows = 2, width = "100%")
  )
}
...
study_ui <- function(id) {
  ...
  shiny::tagList(
    ...
    bslib::layout_columns(
      ...
      shiny::textInput(ns("study_code"), "Study code", width = "100%")
    ),
    bslib::layout_columns(
      col_widths = c(5, 2, 2, 3),
      shiny::textInput(ns("authors"), "Authors (comma separated)", width = "100%"),
      shiny::textInput(ns("version"), "SAP version", value = "1.0", width = "100%"),
      ...
      shiny::div(
        # NULL, not NA: shiny::numericInput() renders NA as the literal string "NA" in
        # the box. NULL omits the value attribute, which is a genuinely empty
        # field.
        shiny::numericInput(ns("min_cell_count"), "Minimum cell count",
                     value = NULL, min = 0, step = 1, width = "100%"),
        shiny::div(class = "form-text", "Counts below this are suppressed before export.")
      ),
      shiny::dateInput(ns("date"), "Date", value = Sys.Date(), width = "100%")
    ),
    shiny::textAreaInput(ns("background"), "Rationale and background", rows = 4, width = "100%"),
    shiny::textAreaInput(
    ...
    shiny::div(
      ...
      shiny::div(
        shiny::h5("Amendment history", class = "mb-1"),
        shiny::p(class = "text-muted mb-0", "What changed in each version of this SAP.")
      ),
      shiny::actionButton(ns("add_amendment"), "Add amendment",
                   class = "btn btn-outline-primary btn-sm", icon = shiny::icon("plus"))
    ),
    shiny::conditionalPanel(
      condition = sprintf("output['%s'] == 0", ns("n_amendments")),
      empty_state("No amendments recorded yet.")
    ),
    shiny::div(id = ns("amendments"))
  )
}
...
study_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$add_amendment, amendments$add(list(
      version = next_sap_version(input$version),
      date    = as.character(Sys.Date())
    )))

    output$n_amendments <- shiny::renderText(amendments$count())
    shiny::outputOptions(output, "n_amendments", suspendWhenHidden = FALSE)
    ...
    objectives <- shiny::reactiveVal(list())
    shiny::observeEvent(input$objectives, {
    ...
    load <- function(study) {
      ...
      shiny::updateTextInput(session, "study_code", value = study$study_code %||% "")
      shiny::updateTextInput(session, "authors", value = paste(unlist(study$authors), collapse = ", "))
      shiny::updateTextInput(session, "version", value = study$version %||% "1.0")
      ...
      shiny::updateNumericInput(session, "min_cell_count",
                         value = if (is.na(mcc)) "" else mcc)
      if (!is.null(study$date)) {
        shiny::updateDateInput(session, "date", value = as.Date(study$date))
      }
      shiny::updateTextAreaInput(session, "background", value = study$background %||% "")
      shiny::updateTextAreaInput(session, "aim", value = study$aim %||% "")
      ...
      shiny::updateTextAreaInput(session, "objectives",
